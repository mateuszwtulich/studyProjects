...
template<class T>
void CGeneticAlgorithm<T>::create_starting_pop()
{
	int i_size = c_my_knaps_problem->getNumberOfItems();

	for (int i = 0; i < i_popul_size; i++)
	{
		CIndividual<T> *pc_indi = new CIndividual<T>(i_size);
...
void CGeneticAlgorithm<T>::v_run_algorithm()
{
	...
	for (int i = 0; i < i_popul_size / 2; i++)
	{
		...
		{
			CIndividual<T> *pc_first_child = new CIndividual<T>(c_my_knaps_problem->getNumberOfItems()); 
			CIndividual<T> *pc_second_child = new CIndividual<T>(c_my_knaps_problem->getNumberOfItems());
			*pc_first_child = *pc_first_parent + *pc_second_parent;
			*pc_second_child = *pc_second_parent + *pc_first_parent;
			c_vector_new_pop.push_back(pc_first_child);
			c_vector_new_pop.push_back(pc_second_child);
			cout << endl << "In cross1: " << (pc_first_child)->toString();
			cout << "In cross2: " << (pc_second_child)->toString();
