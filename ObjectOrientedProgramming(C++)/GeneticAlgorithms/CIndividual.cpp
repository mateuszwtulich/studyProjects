...
template<class T>
CIndividual<T>::CIndividual(int iSize)
{
	i_size = iSize;
...
/*CIndividual& CIndividual::operator =(const CIndividual &cIndividual)
{
	CIndividual *pc_indi = new CIndividual(cIndividual.i_size);

	for (int i = 0; i < i_size; i++)
	{
		pc_indi->pi_genotype[i] = cIndividual.pi_genotype[i];
	}
	delete &cIndividual;
	return *pc_indi;
}*/
...
{
	CIndividual *pc_frist_child = new CIndividual(i_size);
